...
- [outlook_client.py](../outlook_client.py) — Outlook COM wrapper. `get_unread_emails()` / `get_recent_emails(limit)` return `MailInfo` (dataclass, includes a live `item` COM reference — never serialize `item` itself, use `entry_id` + `get_mail_item_by_entry_id()` to reconnect later). Both only look at the default **Inbox** folder (`OL_FOLDER_INBOX`) - mail an Outlook rule has auto-filed into a subfolder or a different top-level folder is NOT seen by this tool. `get_unread_emails()` is capped at `max_unread_results` (config.json, default 200 - most recent) so a large unread backlog can't overload the scoring step.
